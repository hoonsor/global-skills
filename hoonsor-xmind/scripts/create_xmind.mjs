...
function buildZip(files) {
    ...
    for (const { name, data } of files) {
        ...
        localHeader.writeUInt16LE(0x0800, 6);        // flags: bit 11 = UTF-8 encoding (Language Encoding Flag)
        ...
        cdHeader.writeUInt16LE(0x0800, 8);         // flags: bit 11 = UTF-8 encoding (Language Encoding Flag)
...
async function main() {
    ...
    // Support --input <file> to read JSON directly (bypasses shell pipe encoding issues)
    const inputIdx = process.argv.indexOf('--input');
    if (inputIdx !== -1 && process.argv[inputIdx + 1]) {
        const inputPath = resolve(process.argv[inputIdx + 1]);
