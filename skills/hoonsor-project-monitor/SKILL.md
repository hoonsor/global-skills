...
- 使用者輸入「#狀態」：代表「請將前面我提到的內容/進度，更新並推送（部署）到網站上【https://aiprohub.vercel.app/ 】」。
- 使用者要求掃描/監控所有專案狀態
- 使用者要求掃描全域技能或工作流
...
當使用者輸入「#狀態」時，依序執行以下操作：
1. **解析當前對話的變更指示（防呆與確認）**：
   - 解析**含有「#狀態」的那單一次對話內容**，識別使用者要求同步並更新至網站上的具體內容。
   - **範例 1**：「因為新增了【#架構】、【#開工】、【#收工】、【#狀態】指令， #狀態」➡️ 代表需掃描並將這幾個新指令（技能/工作流）的內容推送更新至網站。
   - **範例 2**：「全域技能目錄的架構有所變更， #狀態」➡️ 代表需重新掃描全域技能目錄結構，並將更新內容推送至網站。
   - **範例 3**：「[某專案] 完成了某功能， #狀態」➡️ 代表需要將該專案進度更新至 `PROJECT_STATUS.md` 並同步推送。
   - **確認機制 (重要)**：如果該單次對話的說明不夠明確，或助理無法確定要更新哪一部分（例如只單純輸入了 `#狀態`），**請立刻向使用者提出問題以進行確認**。確認意圖後再執行後續步驟。
2. **執行對應的掃描同步**：
   - 根據分析（或與使用者確認）後的意圖，執行對應的掃描腳本。如果是更新技能，執行 `scan_skills.py`；如果是更新專案，執行 `scan_projects.py` 或 `generate_status.py`。
3. **自動 Git 提交與 Push 部署**：
   - 切換工作目錄至監控網站專案目錄 `D:\01-Project\08-監控AI各專案進度之網站`。
   - 執行 `git add -A`
   - 執行 `git commit -m "feat(status): sync [使用者提及的變更內容] via #狀態"`
   - 執行 `git push origin main` 將最新資料推送到 GitHub，自動觸發 Vercel 的部署，更新網站（https://aiprohub.vercel.app/ ）。
