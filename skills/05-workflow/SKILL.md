...
## #初始化
當使用者輸入「#初始化」指令時，依序執行以下 Git 初始化、遠端倉庫連結與 Vercel 部署工作流：

1. **已存在 Git 檢測與分流**：
   - 先檢查當前專案資料夾是否已包含 Git 設定（例如包含 `.git` 目錄或 `git status` 可正常運行）。
   - **分流 A：已存在 Git 設定，且已配置遠端倉庫（曾上傳/有 remote URL）**：
     - 檢查專案是否為網站或網頁專案。
     - **若是網站專案**：向使用者詢問：「*偵測到此專案已配置遠端倉庫且為網站專案，請問是否需要為您部署到 Vercel？*」
     - 部署流程完成後，停止後續的初始化動作。
   - **分流 B：已存在 Git 設定，但未配置任何遠端倉庫（沒有 remote URL）**：
     - 若使用者輸入中**沒有提供對應的遠端倉庫網址**：
       - 向使用者詢問：「*偵測到專案已有本地 Git 設定，但尚未配置遠端倉庫。請問是否需要為您在 GitHub 上建立新倉庫並部署至 Vercel？*」
       - 待使用者確認後，執行 GitHub 建庫、本地關聯 remote、推送與 Vercel 部署流程。
     - 若使用者輸入中**已提供遠端倉庫網址**：
       - 將該網址設為遠端 origin，進行首次推送，並依序執行 Vercel 部署流程。
   - **分流 C：完全沒有 Git 設定**：
     - 依序執行下方第 2 步至第 5 步的標準初始化流程。
