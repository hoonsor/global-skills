...
## 觸發條件 (Trigger)
當使用者輸入「#同步」、要求「同步技能」、「備份技能」、「推送到遠端技能庫」，或者直接提到要將全域技能推送到 GitHub (如 `hoonsor/global-skills`) 時，請觸發此技能。
...
2. **同步前：複製全域設定檔與知識庫至備份區**：
   為了將 `gemini.md` 與學習錯誤的知識檔一起納入版控，請使用 PowerShell 的 `Copy-Item` 指令，將外層檔案複製到 `skills` 內的 `_meta_backups` 資料夾。
   - 來源 1：`C:\Users\<username>\.gemini\antigravity\gemini.md` -> 目的：`skills\_meta_backups\gemini.md`
   - 來源 2：`C:\Users\<username>\.gemini\antigravity\knowledge\hoonsor-error-learning` -> 目的：`skills\_meta_backups\hoonsor-error-learning` (記得使用 `-Recurse` 與 `-Force` 確保完整覆蓋)

3. **確認 Git 狀態並初始化 (如果尚未初始化)**：
...
5. **自動產出或更新 `README.md` (可選)**：
   若目錄下沒有 `README.md`，可以簡單生成一個，內容說明這是全域技能庫的備份，並列出當前資料夾內所有的技能清單。

6. **執行雙向同步 (Bidirectional Sync)**：
   為了確保本地與遠端保持最新且不遺漏任何變更，請依序執行以下 Git 同步命令：
   - **暫存與提交本地變更** (如果有)：
     先執行 `git add -A`
     再執行 `git commit -m "chore: auto-sync local changes and meta backups"` (若無變更會忽略)
   - **拉取遠端最新技能**：
...
7. **同步後：還原遠端更新的設定檔與知識庫**：
   同步完成後，如果從遠端拉取了最新版本，必須將 `_meta_backups` 中的檔案反向覆蓋回原本的外層路徑，以確保當前裝置套用最新的全域設定與學習知識：
   - 來源 1：`skills\_meta_backups\gemini.md` -> 目的：`C:\Users\<username>\.gemini\antigravity\gemini.md`
   - 來源 2：`skills\_meta_backups\hoonsor-error-learning\*` -> 目的：`C:\Users\<username>\.gemini\antigravity\knowledge\hoonsor-error-learning\`
